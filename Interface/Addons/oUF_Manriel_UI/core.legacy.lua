...
local UnitSpecific = {
	player = function(self)
		...
		self.PvP:SetPoint('CENTER', self, 'TOPRIGHT', 0, -height*0.2)
		...
		Buffs:SetSize((8 + height*0.7) * 16, (16 + height*0.7) * 2);
		...
		Buffs.CreateIcon = UI.methods.CreateIcon
		Buffs.SetPosition = UI.methods.SetPosition
		Buffs.PreUpdate = UI.methods.PreUpdate
		...
		ExperienceFrame.MODIFIER_STATE_CHANGED = function(self, key, state)
			if ((key == 'LALT' or key == 'RALT') and state == 1) then
				ExperienceFrame:Hide(); ReputationFrame:Show();
			elseif ((key == 'LALT' or key == 'RALT') and state == 0) then
				ExperienceFrame:Show(); ReputationFrame:Hide();
			end
		end

		ExperienceFrame:RegisterEvent("MODIFIER_STATE_CHANGED")
		ExperienceFrame:SetScript("OnEvent", function(self, event, ...) ExperienceFrame[event](self, ...) end)
	end,
	...
	target = function(self)
		...
		self.PvP:SetPoint('CENTER', self, 'TOPLEFT', 0, -height*0.2)
...
local Style = function(self, unit)
	...
	PvP:SetSize(height*0.6, height*0.6)
	self.PvP = PvP
